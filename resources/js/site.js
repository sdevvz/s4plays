...
/* ========================================
   Download Tracking Module (Optional)
   ======================================== */
    ...
    init() {
        ...
        downloadButtons.forEach(btn => {
            btn.addEventListener('click', (e) => {
                const appName = btn.closest('.glass-card')?.querySelector('h3')?.textContent || 'Unknown';
                const lockerType = btn.getAttribute('data-locker-type') || 'unknown';
                const lockerUrl = btn.getAttribute('href');
                
                // Track the click (send to analytics if needed)
                this.trackDownload(appName, lockerType, lockerUrl);
                
                // You can add custom logic here
                console.log(`Download initiated: ${appName} via ${lockerType}`);
            });
        });
    },

    trackDownload(appName, lockerType, lockerUrl) {
        // Example: Send to Google Analytics
        if (typeof gtag !== 'undefined') {
            gtag('event', 'download_click', {
                'app_name': appName,
                'locker_type': lockerType,
                'locker_url': lockerUrl
            });
        }
        ...
        //     body: JSON.stringify({ appName, lockerType, lockerUrl })
        // });
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    Sort.init();
    DownloadModal.init();
    DownloadTracking.init();  // Track locker clicks
    SmoothScroll.init();
    NavbarScroll.init();
